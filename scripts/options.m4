...
A @option{--shell-port} is automatically included on the list of ports to be listened at; you do not normally need to specify @option{-p|--listen} for it as well.  However, if you do have both @option{-p|--listen=@var{port},@var{listener}} and @option{--shell-port=@var{port}} for the same non-zero @var{port}, this will force the shell listener be @var{listener} (i.e., rather than the usual hardwired @code{shell_listener}).@footnote{Yes, this is esoteric and weird.  We may rethink this behavior at some point.  Normally you do not want to change the shell listener, but you might if you, say, want to try out a more advanced shell listener@dots{}.  On the other hand, you can probably just change the value of @code{$shell_listener} in the database, since you're likely already messing with the database anyway.}
]dnl
dnl ====================================================
